...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
   ...
   * so you can easily start playing or watching new streams without
  ...
  public void stopAll() {
    ...
    // Clean up all renderers by setting their streams to null
    for (int i = 0; i < renders.size(); i++) {
      FlutterRTCVideoRenderer renderer = renders.valueAt(i);
      if (renderer != null) {
  ...
  public void streamDispose(final String streamId) {
    ...
    if (stream != null) {
      ...
      removeStreamForRendererByIdAnyOwner(streamId);
    } else {
  ...
  private void removeStreamForRendererById(String streamId) {
    Log.d(TAG, "removeStreamForRendererById() for streamId: " + streamId);
    for (int i = 0; i < renders.size(); i++) {
      FlutterRTCVideoRenderer renderer = renders.valueAt(i);
      if (renderer.checkMediaStream(streamId, "local")) {
        Log.d(TAG, "Found local renderer for streamId: " + streamId + ", setting stream to null");
        renderer.setStream(null, "");
      }
    }
  }

  private void removeStreamForRendererByIdAnyOwner(String streamId) {
    Log.d(TAG, "removeStreamForRendererByIdAnyOwner() for streamId: " + streamId);
    // Collect renderers to remove to avoid ConcurrentModificationException
    List<Long> renderersToRemove = new ArrayList<>();
    for (int i = 0; i < renders.size(); i++) {
      FlutterRTCVideoRenderer renderer = renders.valueAt(i);
      // Check if renderer has this stream regardless of owner tag
      String rendererStreamId = renderer.getMediaStreamId();
      if (rendererStreamId != null && streamId.equals(rendererStreamId)) {
        Log.d(TAG, "Found renderer for streamId: " + streamId + " with ownerTag: " + renderer.getOwnerTag()
            + ", disposing and marking for removal");
        renderer.Dispose(); // Call Dispose to release EglRenderer resources
        renderersToRemove.add(renders.keyAt(i));
      }
    }

    // Remove disposed renderers from the renders list
    for (Long key : renderersToRemove) {
      renders.remove(key);
    }
  }
  ...
  @Override
  public void removeStreamFromAllRenderers(String streamId) {
    removeStreamForRendererByIdAnyOwner(streamId);
  }
  ...
  /**
   * Disposes the GetUserMediaImpl resources, releasing all hardware resources
   * (camera, etc.)
   * This is useful when you want to free up resources without destroying the
   * entire WebRTC connection
   */
